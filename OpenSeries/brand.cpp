...
namespace brand {
	...
	static constexpr float BRAND_Q_RANGE = 1035;
	...
	bool castR(const game_object_script& target, const std::string& mode)
	{
		...
		if (myhero->get_active_spell() && myhero->get_active_spell()->cast_start_time() + 0.1 > gametime->get_time() && myhero->get_active_spell()->get_spellslot() == spellslot::r) return false;
	...
	float getQDamage(const game_object_script& target)
	{
		...
		const float damage = 50 + spell->level() * 30 + myhero->get_total_ability_power() * 0.65;
	...
	float getEDamage(const game_object_script& target)
	{
		...
		const float damage = 30 + 30 * spell->level() + myhero->get_total_ability_power() * 0.6;
		const float damageLibDamage = damagelib->calculate_damage_on_unit(myhero, target, damage_type::magical, damage);
		...
		for (auto& obj : particlePredList)
		{
			...
			if (obj.isTeleport)
			{
				...
				if (obj.target && obj.obj->get_position().distance(obj.target->get_position()) <= 0) {
					obj.castingPos = navmesh->get_nearest_passable_cell_center(obj.target->get_position().extend(nexusPos, obj.target->get_bounding_radius() + obj.owner->get_bounding_radius()));
				}
				else
				{
